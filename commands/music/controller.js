...
module.exports = {
    ...
    async execute({ inter }) {
        ...
        if (channel.type !== ChannelType.GuildText) {
            return inter.editReply({ content: await Translate(`You need to send it to a text channel ❌`) });
        }
        ...
        const embed = new EmbedBuilder()
            ...
            .setColor('#2f3136')
            .setFooter({ 
                text: await Translate('Phucx Nè ❤️'), 
        ...
        const row3 = new ActionRowBuilder().addComponents(lyrics, shuffle, clear, queuebutton, np);

        try {
            await channel.send({ embeds: [embed], components: [row1, row2, row3] });
            await inter.editReply({ content: await Translate(`Controller sent to ${channel} ✅`) });
        } catch (error) {
            console.log(`Controller send error: ${error.message}`);
            await inter.editReply({ content: await Translate(`Failed to send controller ❌`) });
        }
    }
}